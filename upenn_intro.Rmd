...
```{r simplestExamples, echo=TRUE, eval=FALSE}
...
```{r spacesDontCount, eval=FALSE, tidy=FALSE}
...
```{r multiLineStmnt1, eval=FALSE, tidy=FALSE}
...
```{r oneLineManyStatements, eval=FALSE, tidy=FALSE}
...
```{r numericOperations, eval=FALSE}
...
```{r fnYouCannotBeSerious, eval=FALSE, tidy=FALSE}
...
```{r functions1, eval=FALSE}
  ...
  zFn <- function(w) {  ## write our own function to do the above
      wRt <- sqrt(w)
      wRt * 42
  }
...
```{r showPackagesNowLoaded, eval=FALSE}
...
```{r checkSyntax, eval=FALSE, tidy=FALSE}
...
```{r indexRepeat, eval=FALSE, tidy=FALSE}
...
```{r subsetAssign, eval=FALSE}
  ...
  a[c(1,3)] <- c(100, 200);  a
  a[-2] <- c(1000, 2000);  a
  a[c(FALSE, TRUE, TRUE)] <- c(10000, 20000);  a
...
```{r miscVec, eval=FALSE, tidy=FALSE}
...
```{r noVectorBounds, eval=FALSE, tidy=FALSE}
...
```{r assocArray, eval=FALSE}

  salaries <- c(John=35000, Brenda=100000, Jim=12000)
  salaries
...
```{r assocArrayGotcha, eval=FALSE}

  salaries <- c(35000, 100000, 12000)
...
```{r numericFns, eval=FALSE}
  ...
  z <- c(3 + 4i, 6 + 8i)  ## but R DOES handle complex numbers
  sqrt(z)
  ##
  log(x^2)
  exp(1)
...
```{r matrixBackAndForth, eval=FALSE}
...
```{r subsetMatrix, eval=FALSE, tidy=FALSE}
...
```{r subsetMatrixByName, eval=FALSE, tidy=FALSE}
...
```{r patternedMatrices, eval=FALSE, tidy=FALSE}
...
```{r formatOfPrintedList, eval=FALSE}
...
```{r listAccess1, eval=FALSE, tidy=FALSE}
...
```{r listAccess2, eval=FALSE, tidy=FALSE}
...
```{r accessSublist1, eval=FALSE, tidy=FALSE}
...
```{r accessSublist2, eval=FALSE, tidy=FALSE}
...
```{r makeDF1, eval=FALSE, tidy=FALSE}
...
```{r opsOnDF, eval=FALSE, tidy=FALSE}
...
```{r matMul, eval=FALSE, tidy=FALSE}
...
```{r readTableURL, eval=FALSE, tidy=FALSE}
...
```{r forLoop1, eval=FALSE, tidy=FALSE}
...
```{r whileLoop1, eval=FALSE, tidy=FALSE}
...
```{r repeatLoop, eval=FALSE, tidy=FALSE}
...
```{r funSemantics1, eval=FALSE, tidy=FALSE}
...
```{r funSemantics2, eval=FALSE, tidy=FALSE}
...
```{r funSyntax2, eval=FALSE, tidy=FALSE}
...
```{r funRealistic1, eval=FALSE, tidy=FALSE}
...
```{r funArgs1, eval=FALSE, tidy=FALSE}
...
```{r funArgsDefault, eval=FALSE, tidy=FALSE}
...
```{r returnList, eval=FALSE, tidy=FALSE}
...
```{r funArgsByValue1, eval=FALSE, tidy=FALSE}
...
```{r variableCategories, eval=FALSE, tidy=FALSE}
...
```{r scopeExample1, eval=FALSE, tidy=FALSE}
...
```{r scopeExample2, eval=FALSE, tidy=FALSE}
...
```{r lapply1, eval=FALSE, tidy=FALSE}
...
```{r sapply1, eval=FALSE, tidy=FALSE}
...
```{r sapply2, eval=FALSE, tidy=FALSE}
...
```{r replicate1, eval=FALSE, tidy=FALSE}
...
```{r apply1, eval=FALSE, tidy=FALSE}
...
```{r mapply1, eval=FALSE, tidy=FALSE}
...
```{r do.call.1, eval=FALSE, tidy=FALSE}
...
```{r rbindReminder, eval=FALSE, tidy=FALSE}
...
```{r rbindList1, eval=FALSE, tidy=FALSE}
...
```{r stackDFlapply, eval=FALSE, tidy=FALSE}
...
```{r stringManip1, eval=FALSE, tidy=FALSE}
...
```{r setSeed1, eval=FALSE, tidy=FALSE}
...
```{r parExample1, eval=FALSE, tidy=FALSE}
...
```{r decorateWithPoints, eval=FALSE, tidy=FALSE}
...
```{r multiplePlots1, eval=FALSE, tidy=FALSE}
...
```{r openNewDisplay, eval=FALSE, tidy=FALSE}
...
```{r savePlot1, eval=FALSE, tidy=FALSE}
...
```{r simpleHist, eval=FALSE, tidy=FALSE}
...
```{r battingAverages, tidy=FALSE, fig.cap=''}
...
```{r histDiffBreaks, eval=FALSE}

  par.save <- par(mfrow=c(2, 1), mar=c(2.0, 0, 1, 0))
...
```{r multipleHists1, eval=FALSE, tidy=FALSE}
...
```{r multipleHists2, eval=FALSE, tidy=FALSE}
...
```{r meanNotMedian, eval=FALSE, tidy=FALSE}
...
```{r simpleBoxPlot, eval=FALSE, tidy=FALSE}
...
```{r annotatedBoxplot, eval=FALSE, tidy=FALSE}
...
```{r boxPlotList1, eval=FALSE, tidy=FALSE}
...
```{r boxPlotList2, eval=FALSE, tidy=FALSE}
...
```{r multipleBoxPlots1, eval=FALSE, tidy=FALSE}
...
```{r introspection1, eval=FALSE, tidy=FALSE}
...
```{r theIsQueries1, eval=FALSE, tidy=FALSE}
...
```{r theIsQueries2, eval=FALSE, tidy=FALSE}
...
```{r findTheIsFns, eval=FALSE, tidy=FALSE}
...
```{r queryNames1, eval=FALSE, tidy=FALSE}
...
```{r regExprHelp, eval=FALSE, tidy=FALSE}
...
```{r grepSimpleEx, eval=FALSE, tidy=FALSE}
...
```{r regexprExample, eval=FALSE, tidy=FALSE}
...
```{r regexprVsgregexpr, eval=FALSE, tidy=FALSE}
...
```{r subgsubEx1, eval=FALSE, tidy=FALSE}
...
```{r strsplitEx1, eval=FALSE, tidy=FALSE}
...
```{r moreRealisticRegex1, eval=FALSE, tidy=FALSE}
...
```{r helpRegexp, eval=FALSE, tidy=FALSE}
...
```{r characterClass1, eval=FALSE, tidy=FALSE}
...
```{r regexpCharClass1, eval=FALSE, tidy=FALSE}
...
```{r repQuant1, eval=FALSE, tidy=FALSE}
...
```{r regexpRemember1, eval=FALSE, tidy=FALSE}
...
```{regexpOR1, eval=FALSE, tidy=FALSE}
...
```{r options1, eval=FALSE, tidy=FALSE}
