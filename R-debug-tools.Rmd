...
```{r warningExample, prompt=TRUE, results='markup'}
  zzz <- log(-1)
  zzz
```
...
```{r trace-h, eval=FALSE}

  trace(h, quote(if(is.nan(r)) { browser() }), at=3)
  f(7)
  f(-3)
 untrace(f)
